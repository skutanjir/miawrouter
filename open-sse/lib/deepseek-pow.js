// deepseek-pow.js — pure-JS DeepSeekHashV1 proof-of-work solver.
//
// Dependency-free ESM port of OmniRoute's solver (open-sse/lib/deepseek-pow.ts +
// deepseek-pow-solver.cjs). OmniRoute prefers a WASM accelerator; this port ships
// the JS fallback only so the module has zero dependencies and no binary asset.
// Slower (~5-6s at difficulty 144000) but functionally identical, and the WASM
// path can be layered in later without touching callers.
//
// The bundled solver below is the minified Keccak sponge implementation from
// OmniRoute verbatim (it only ever needs Node's global `Buffer`).

function loadSolver() {
  const cjsModule = { exports: {} };
  // ── bundled solver (verbatim from OmniRoute open-sse/lib/deepseek-pow-solver.cjs) ──
  ...
  cjsModule.exports={U};
  return cjsModule.exports.U;
}

const U = loadSolver();

// Shared hash wrapper matching OmniRoute's solveWithJS: Keccak-256 sponge,
// 6-byte squeeze → 12-char hex digest compared against the challenge.
...
// Sync wrapper kept for backward compat (same JS solver).
