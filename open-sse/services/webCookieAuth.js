/**
 * Shared web-cookie parsing helpers, ported from OmniRoute's
 * src/lib/providers/webCookieAuth.ts (pure JS, no dependencies).
 */
 ...
 * the browser earned. Like `sso-rw`, each is appended only when it appears as a
 * real cookie pair, so a bare `sso` blob still produces exactly `sso=<value>`
 * (no phantom cf keys).
 *
 ...
 * replay, only a bearer credential (handled by extractQwenToken).
 */
