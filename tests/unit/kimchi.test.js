...
describe("kimchi registry entry", () => {
  ...
  it("is a free-tier provider whose only auth mode is oauth", () => {
    expect(kimchiEntry.id).toBe("kimchi");
    // Kimchi is an oauth-only *free tier* card. The providers grid documents
    // this in providers/page.js ("Free Tier cards may be oauth-only (e.g.
    // kimchi)") and connects it through authModes + dualAuthTypes, so the
    // category stays "freeTier" rather than moving to the oauth grid.
  ...
  it("points at the OpenAI-compatible gateway with an authenticated UA", () => {
    ...
    expect(typeof ua === "string" && ua.length > 0, `User-Agent missing: ${ua}`).toBe(true);
  });
  ...
  it("exposes the upstream static models", () => {
    ...
    expect(ids.length, `expected >= 5 static models, got ${ids.length}`).toBeGreaterThanOrEqual(5);
  });
...
// ── Pure-function clones of the service logic. The real modules
//     (src/lib/oauth/services/kimchi.js) pull the Next.js server dependency
//     graph, so the decision logic is mirrored here and tested in isolation. ──
...
describe("kimchiModels", () => {
  it("maps Kimchi metadata entries to MiawRouter model shape", () => {
    const raw = [{
      slug: "glm-5.2-fp8",
