...
Integrate MiawRouter with Cline VSCode extension to route your AI requests through MiawRouter's intelligent routing system.
...
Set the base URL to your MiawRouter endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your MiawRouter endpoint
...
3. The key should start with `sk-miawrouter-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your MiawRouter configuration
...
4. Cline will use MiawRouter to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using MiawRouter
...
3. Get AI-powered explanations through MiawRouter

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use MiawRouter to understand context and make changes
...
2. Verify your MiawRouter instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check MiawRouter logs for any errors
...
1. In Cline settings, set Base URL to: `https://miawrouter.web.id`
2. Make sure you have configured your API key in the MiawRouter cloud dashboard
...
## Integration with MiawRouter Features

### Model Routing

MiawRouter automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, MiawRouter automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through MiawRouter dashboard:
- Total requests
- Token usage
- Cost per model
