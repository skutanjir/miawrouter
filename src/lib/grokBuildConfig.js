...
const previousDefaultRegExp = /^# miawrouter-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# miawrouter-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function parseModelSection(toml, slot) {
  const resolved = sectionRegExp(`model.${slot}`).test(toml) ? slot : null;
...
function restorePreviousDefault(toml) {
  ...
  if (currentDefault === GROK_MAIN_MODEL_SLOT) {
